...
/**
 * Stores Google OAuth app credentials in a database table.
 *
...
{
    protected string $table = 'google_configurations';
    ...
    public function save( array $credentials ): void
    {
        $row = [
            ...
            'client_secret' => isset( $credentials[ 'client_secret' ] )
                ? $this->encrypter->encryptString( $credentials[ 'client_secret' ] )
                : null,
    ...
    protected function load(): array
    {
        ...
        if ( ! empty( $row->client_secret ) ) {
            try {
                ...
                Log::warning(
                    'artisanpack-ui/google: failed to decrypt stored client_secret; treating as unconfigured. Was APP_KEY rotated without re-encrypting the row?',
